...
const AdminDashboard: React.FC = () => {
  ...
  const [filter, setFilter] = useState<"ALL" | "PENDING" | "VALIDATED">(
    "PENDING",
  );
  ...
  const handleSaveLink = async () => {
    setSavingLink(true);
    await api.updateMeetLink(meetLink);
    setSavingLink(false);
    alert("Lien Google Meet mis à jour !");
  };
  ...
  const filteredData = registrations.filter((r) => {
    ...
    if (filter === "VALIDATED")
      return r.status === "VALIDATED" || r.status === "FREE_ACCESS";
    return true;
  });
      ...
      case "FREE_ACCESS":
        return (
          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400 border border-blue-200 dark:border-blue-800">
            <ShieldCheck className="w-3 h-3 mr-1" /> Gratuit
          </span>
        );
    }
  };
  ...
  const getTypeBadge = (type?: string) => {
    if (type === "KAIZEN") {
      return (
        <span
          className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 border border-blue-200 dark:border-blue-800 ml-2"
          title="Preuve: Capture d'écran"
        >
          <Smartphone className="w-3 h-3 mr-1" /> ABONNÉ KAIZEN
        </span>
      );
    } else {
      // Default to CCP if undefined (legacy)
      return (
        <span
          className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300 border border-purple-200 dark:border-purple-800 ml-2"
          title="Preuve: Reçu 3000 DA"
        >
          <Wallet className="w-3 h-3 mr-1" /> 3000 DA (CCP)
        </span>
      );
    }
  };
    ...
    <div className="w-full max-w-6xl mx-auto p-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm">
            Gérez les inscriptions et les paramètres.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm mb-8">
        ...
        <div className="flex gap-4">
          <input
            ...
            className="flex-grow px-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary-500 focus:outline-none dark:text-white"
          />
          <button
            onClick={handleSaveLink}
            disabled={savingLink}
            className="flex items-center px-5 py-2 bg-primary-600 text-white rounded-xl hover:bg-primary-700 transition-colors font-medium disabled:opacity-70"
          >
            {savingLink ? (
        ...
        <p className="text-xs text-slate-500 mt-2 ml-1">
          Ce lien s'affichera automatiquement pour les utilisateurs validés.
        </p>
      </div>
      ...
      <div className="flex justify-end mb-4">
        <div className="flex bg-white dark:bg-slate-800 p-1 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
          ...
          >
            En attente{" "}
            <span className="ml-1 px-1.5 py-0.5 bg-primary-200 dark:bg-slate-600 rounded-full text-[10px]">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400 flex items-center">
                    ...
                    <span className="text-xs">
                      Inscrit le {new Date(reg.date).toLocaleDateString()}
                    </span>
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 w-full md:w-auto mt-2 md:mt-0 pl-14 md:pl-0">
                ...
                {reg.status !== "PENDING" && reg.status !== "FREE_ACCESS" && (
                  <div className="flex gap-2">
                    ...
                    <button
                      disabled
                      className="text-emerald-600 dark:text-emerald-500 text-sm font-medium flex items-center px-4 py-2 bg-emerald-50 dark:bg-emerald-900/10 rounded-lg border border-emerald-100 dark:border-emerald-900/20 cursor-default"
                    >
                      <Check className="w-4 h-4 mr-2" /> Validé
                    </button>
                  </div>
                )}
