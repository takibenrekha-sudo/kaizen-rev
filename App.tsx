...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const checkAdminRoute = () => {
      // Check if URL contains /admin, #admin or ?admin
    ...
    // Listen for hash changes (e.g. if user types #admin in URL bar)
  ...
  const handleHomeClick = () => {
    ...
    setUserData(null);
    // Clear URL to remove admin trace so refresh goes back to home
  ...
  const handleReceiptSubmit = async (file: File) => {
    ...
    try {
      await api.sendReceipt(userData, file);
  ...
  const handleAdminLoginCancel = () => {
    setStep(AppStep.FORM);
    // Clear URL parameters/hash if user cancels login
    if (window.history.pushState) {
        window.history.pushState({}, '', window.location.pathname.replace('/admin', '/'));
    }
  };
      ...
      case AppStep.CHECKING:
        return (
          <div className="w-full transition-all duration-500 ease-in-out">
            <InscriptionForm onSubmit={handleUserSubmit} isLoading={isLoading} />
          </div>
        );
  ...
  return (
    <div className={`flex flex-col min-h-screen transition-colors duration-300 bg-slate-50 dark:bg-slate-950`}>
      <Header 
        darkMode={darkMode} 
